...
float LifetimeCorrectionFactor(const std::vector<float> &detAnodes,
                               const float inputPos, const float lifetime,
...
float eVisWithRecombination(
    const ParameterStruct &parameters, const float inputQ,
    const float dEdx_use = 2. /*MeV/cm "dEdxMIP" from FLOW code*/) {
  const float wIon =
      23.6 / 1.0e6; // MeV/e-, a hard-coded for now value used later

  // MIP Recombination with the Q->E calculation as in FLOW file
  // see e.g.
  // https://github.com/DUNE/ndlar_flow/blob/develop/src/proto_nd_flow/reco/charge/calib_prompt_hits.py#L289
  float recomb = 1.;
  if (parameters.fBoxRecombination) {
    float csi = parameters.fBoxBeta * dEdx_use /
                (parameters.fEField * parameters.fDensity);
    recomb = TMath::Log(parameters.fBoxAlpha + csi) / csi;
  } else if (parameters.fBirksRecombination) {
    recomb = parameters.fBirksA /
             (1. + parameters.fBirksK * dEdx_use /
                       (parameters.fEField * parameters.fDensity));
  }
...
float dEdxWithRecombination(
    const ParameterStruct &parameters, const float inputdQdx,
    const float dEdx_use = 2. /*MeV/cm "dEdxMIP" from FLOW code*/) {
  float dEdx_val = 0.;
  const float wIon =
      23.6 / 1.0e6; // MeV/e-, a hard-coded for now value used later
  ...
  if (parameters.fShouldCorrectRecomb) {
    if (parameters.fFlowStyleRecombination) {
      ...
      // Box style, LArSoft style, angular part turned off, we'll just use the
      // box beta as-is
      // https://github.com/LArSoft/larreco/blob/develop/larreco/Calorimetry/CalorimetryAlg.cxx
      dEdx_val = (TMath::Exp(parameters.fBoxBeta * wIon * inputdQdx) -
                  parameters.fBoxAlpha) /
                 parameters.fBoxBeta;
    } else if (parameters.fBirksRecombination) {
      // Birks style, LArSoft style
      // https://github.com/LArSoft/larreco/blob/develop/larreco/Calorimetry/CalorimetryAlg.cxx
      dEdx_val =
          inputdQdx / (parameters.fBirksA / wIon -
                       parameters.fBirksK / parameters.fEField * inputdQdx);
    }
  } else {
    // Assume recomb = 1?
    float recomb = 1.;
...
float pFromRange_proton(const float inputRange) {
  ...
  if (inputRange > 0 && inputRange <= 80)
    ...
    KE = 149.904 + (3.34146 * inputRange) +
         (-0.00318856 * inputRange * inputRange) +
         (4.34587E-6 * inputRange * inputRange * inputRange) +
         (-3.18146E-9 * inputRange * inputRange * inputRange * inputRange) +
         (1.17854E-12 * inputRange * inputRange * inputRange * inputRange *
          inputRange) +
         (-1.71763E-16 * inputRange * inputRange * inputRange * inputRange *
...
void ProcessPostReco(const ParameterStruct &parameters) {
  ...
  if (parameters.fDetector == 0) {
    // NDLAr anodes -- these are approximately the mid points of each anode.
    // Could try to use each side of this a few cm apart instead.
    ...
    posAnodes.push_back(250.);
    posAnodes.push_back(350.);
    // NDLAr edges -- these are approximate and based on MicroProd N3p4 hit
    // positions with 0.5 cm bins
    ...
    xBoundaries.push_back(63.9273);
    // Y and Z boundaries from looking at hit positions in MiniRun 6.4 with 0.5
    // cm bins
  ...
  if (!fileSource) {
    std::cout << "Error in ProcessPostReco(): can't open file "
  ...
  std::unique_ptr<LArRecoNDFormat> pandoraIn =
      std::make_unique<LArRecoNDFormat>(recoTree);

  long nEntries = recoTree->GetEntries();

  std::cout << "Runninng ProcessEvents on " << nEntries
            << " entries with pixel pitch " << parameters.pixelPitch
            << " and track/shower separation score of "
            << parameters.trackScoreCut << std::endl;
  ...
  for (long entryIdx = 0; entryIdx < nEntries; ++entryIdx) {
    ...
    std::vector<float> trkStartX, trkStartY, trkStartZ, trkEndX, trkEndY,
        trkEndZ;
    std::vector<float> trkStartDirX, trkStartDirY, trkStartDirZ, trkEndDirX,
        trkEndDirY, trkEndDirZ;
    std::vector<float> trkLen, trk_KEFromLength_muon, trk_KEFromLength_proton,
    ...
    std::vector<float> shwrCentroidX, shwrCentroidY, shwrCentroidZ, shwrStartX,
        shwrStartY, shwrStartZ;
    ...
    for (unsigned int particleIdx = 0; particleIdx < nParticles;
         ++particleIdx) {
      ...
      // Read in the vertex and point vector that will be the input to the track
      // and shower fits
      int sliceID = pandoraIn->m_sliceID->at(particleIdx);
      int clusterID = pandoraIn->m_clusterID->at(particleIdx);
      CartesianVector vertexVector(pandoraIn->m_nuVtxX->at(particleIdx),
                                   pandoraIn->m_nuVtxY->at(particleIdx),
                                   pandoraIn->m_nuVtxZ->at(particleIdx));
      CaloHitList caloHitList;
      for (unsigned int idxHits = 0; idxHits < pandoraIn->m_recoHitId->size();
           ++idxHits) {
        if (pandoraIn->m_recoHitSliceId->at(idxHits) == sliceID &&
            pandoraIn->m_recoHitClusterId->at(idxHits) == clusterID) {
          // Skip hit if it fails the threshold
          if (parameters.applyThreshold &&
              pandoraIn->m_recoHitE->at(idxHits) < parameters.thresholdVal)
            continue;
          CartesianVector thisHit(pandoraIn->m_recoHitX->at(idxHits),
                                  pandoraIn->m_recoHitY->at(idxHits),
                                  pandoraIn->m_recoHitZ->at(idxHits));
          lar_content::LArCaloHitParameters chParams;
          chParams.m_positionVector = thisHit;
          chParams.m_expectedDirection =
      ...
      // Fit it as a track?
      if (!parameters.runTrackFit || (parameters.trackScoreCut > 0. &&
                                      trackScore < parameters.trackScoreCut)) {
        // if we aren't running the track fit, then fill defaults for the track
        // parameters we expect for every reco particle track values
      ...
      if (parameters.runTrackFit && (parameters.trackScoreCut < 0. ||
        ...
        // TODO: Make the MinTrajectoryPoints(default=2) and
        // SlidingFitHalfWindow(20) configurable
        ...
        bool filledPID = false; // We'll check at the end and fill PID with
                                // bogus info if PID info not filled
        ...
        try {
          lar_content::LArPfoHelper::GetSlidingFitTrajectory(
              &caloHitList, vertexVector, slidingFitHalfWindow,
              parameters.pixelPitch, trackStateVector, &indexVector, true);
          trackStateSuccess = true;
        } catch (const pandora::StatusCodeException &) {
          trackStateSuccess = false;
        }

        // If user has set the Voxelize Z function, then rerun the track fit,
        // starting from the output of the first fit
        ...
        if (parameters.voxelizeZ && trackStateSuccess) {
          if (parameters.verbosity >= 1) {
            std::cout
                << "    INFO: Since voxelization is turned on, we will take "
                   "the output of the track fit and try to voxelize now."
                << std::endl;
            std::cout << "    ----> Input track has " << trackStateVector.size()
          ...
          try {
            ...
            for (unsigned int idxPt = 0; idxPt < trackStateVector.size();
                 ++idxPt) {
              const lar_content::LArTrackState &trackState =
                  trackStateVector.at(idxPt);
              lar_content::LArCaloHitParameters chParams;
              chParams.m_positionVector =
                  trackState.GetCaloHit()->GetPositionVector();
              chParams.m_expectedDirection =
                  pandora::CartesianVector(0.f, 0.f, 1.f);
              chParams.m_cellNormalVector =
                  pandora::CartesianVector(0.f, 0.f, 1.f);
              chParams.m_cellGeometry = pandora::RECTANGULAR;
              chParams.m_cellSize0 = parameters.pixelPitch;
              chParams.m_cellSize1 = parameters.pixelPitch;
              ...
              chParams.m_nCellInteractionLengths = 1.f;
              chParams.m_time = 0.f;
              chParams.m_inputEnergy =
                  trackState.GetCaloHit()->GetInputEnergy();
              chParams.m_mipEquivalentEnergy =
                  trackState.GetCaloHit()->GetInputEnergy();
              chParams.m_electromagneticEnergy =
                  trackState.GetCaloHit()->GetInputEnergy();
              chParams.m_hadronicEnergy =
                  trackState.GetCaloHit()->GetInputEnergy();
              chParams.m_isDigital = false;
              chParams.m_hitType = pandora::TPC_3D;
              chParams.m_hitRegion = pandora::SINGLE_REGION;
              ...
              chParams.m_isInOuterSamplingLayer = false;
              chParams.m_pParentAddress =
                  (void *)(static_cast<uintptr_t>(++hitCounter_v1p5));
              chParams.m_larTPCVolumeId = 0;
              chParams.m_daughterVolumeId = 0;
              lar_content::LArCaloHit *ch =
                  new lar_content::LArCaloHit(chParams);
              caloHitVect_v1.push_back(ch);
            }
            // Now let's construct the version that goes into the second pass
            // track fit.
            // 1. Loop through the vector and for each element, gather all the
            // consecutive elements within epsilon of the z value
            // 2. Within this subset, find the maximum Q hit, start here
            //     a. Gather this hit and the ones within an x, y distance of
            //     the voxel setting b. Make a new calohit that is the weighted
            //     mean of the (x, y, z) of these hits and the sum of the Q
            //     values
            // 3. Repeat on the maximal Q value of the hits letf and continue
            // repeating till all hits are swept up
            // 4. Run track fit on this.
            CaloHitList caloHitList_v2;
            for (unsigned int idxHit = 0; idxHit < caloHitVect_v1.size();
                 ++idxHit) {
              if (parameters.verbosity >= 2)
                std::cout << "      hit idx " << idxHit << " of "
                          << caloHitVect_v1.size() << std::endl;
              // Step 1
              float thisZ =
                  caloHitVect_v1.at(idxHit)->GetPositionVector().GetZ();
              ...
              while (!stopLoop && idxHit < caloHitVect_v1.size() - 1) {
                if (fabs(caloHitVect_v1.at(idxHit + 1)
                             ->GetPositionVector()
                             .GetZ() -
                         thisZ) < std::numeric_limits<float>::epsilon()) {
              ...
              if (parameters.verbosity >= 2)
                std::cout
                    << "      --> At this stage of the voxelization, we have "
                    << caloHitVect_tmp.size() << " hits to possibly merge."
                    << std::endl;
              // Step 2-3
              if (caloHitVect_tmp.size() == 1) {
                lar_content::LArCaloHitParameters chParams;
                chParams.m_positionVector =
                    caloHitVect_tmp.at(0)->GetPositionVector();
                chParams.m_expectedDirection =
                    pandora::CartesianVector(0.f, 0.f, 1.f);
                chParams.m_cellNormalVector =
                    pandora::CartesianVector(0.f, 0.f, 1.f);
                ...
                chParams.m_time = 0.f;
                chParams.m_inputEnergy =
                    caloHitVect_tmp.at(0)->GetInputEnergy();
                chParams.m_mipEquivalentEnergy =
                    caloHitVect_tmp.at(0)->GetInputEnergy();
                chParams.m_electromagneticEnergy =
                    caloHitVect_tmp.at(0)->GetInputEnergy();
                ...
                chParams.m_isInOuterSamplingLayer = false;
                chParams.m_pParentAddress =
                    (void *)(static_cast<uintptr_t>(++hitCounter_v2));
                chParams.m_larTPCVolumeId = 0;
                chParams.m_daughterVolumeId = 0;
                lar_content::LArCaloHit *ch =
                    new lar_content::LArCaloHit(chParams);
                ...
                while (caloHitVect_tmp.size() > 0) {
                  ...
                  float maxQ_X = 0.;
                  float maxQ_Y = 0.;
                  for (unsigned int idxHit_inner = 0;
                       idxHit_inner < caloHitVect_tmp.size(); ++idxHit_inner) {
                    if (caloHitVect_tmp.at(idxHit_inner)->GetInputEnergy() >
                        maxQ) {
                      maxQ = caloHitVect_tmp.at(idxHit_inner)->GetInputEnergy();
                      maxQ_X = caloHitVect_tmp.at(idxHit_inner)
                                   ->GetPositionVector()
                                   .GetX();
                      maxQ_Y = caloHitVect_tmp.at(idxHit_inner)
                                   ->GetPositionVector()
                                   .GetY();
                    }
                  }
                  if (parameters.verbosity >= 2)
                    std::cout << "      --> Max Hit X = " << maxQ_X
                              << ", Y = " << maxQ_Y << ", Q = " << maxQ
                              << std::endl;
                  ...
                  for (unsigned int idxHit_inner = 0;
                       idxHit_inner < caloHitVect_tmp.size(); ++idxHit_inner) {
                    float thisX_inner = caloHitVect_tmp.at(idxHit_inner)
                                            ->GetPositionVector()
                                            .GetX();
                    float thisY_inner = caloHitVect_tmp.at(idxHit_inner)
                                            ->GetPositionVector()
                                            .GetY();
                    if (parameters.verbosity >= 2)
                      std::cout
                          << "      --> This Hit X = " << thisX_inner
                          << ", Y = " << thisY_inner << ", Q = "
                          << caloHitVect_tmp.at(idxHit_inner)->GetInputEnergy()
                          << std::endl;
                    if (std::sqrt(std::pow(thisX_inner - maxQ_X, 2) +
                                  std::pow(thisY_inner - maxQ_Y, 2)) <
                        parameters.voxelZHW) {
                      float thisZ_inner = caloHitVect_tmp.at(idxHit_inner)
                                              ->GetPositionVector()
                                              .GetZ();
                      float thisQ_inner =
                          caloHitVect_tmp.at(idxHit_inner)->GetInputEnergy();
                  ...
                  if (parameters.verbosity >= 2)
                    std::cout << "      --> Making a new hit from " << xs.size()
                              << " hit(s) and deleting " << toDelete.size()
                              << " hits." << std::endl;
                  std::vector<lar_content::LArCaloHit *> caloHitVect_tmp_prev =
                      caloHitVect_tmp;
                  caloHitVect_tmp.clear();
                  for (unsigned int idxCopy = 0;
                       idxCopy < caloHitVect_tmp_prev.size(); ++idxCopy) {
                    bool skipCopy = false;
                    for (unsigned int checkIdx = 0; checkIdx < toDelete.size();
                         ++checkIdx) {
                  ...
                  caloHitList_v2.push_back(ch);
                  if (parameters.verbosity >= 2)
                    std::cout << "      --> After this particular "
                                 "voxelization, we have "
                              << caloHitVect_tmp.size()
                              << " hits remaining to possibly merge.\n"
                              << "          and caloHitList_v2 size is "
                              << caloHitList_v2.size() << std::endl;
                }
              } // Step 2-3
            }   // Steps 1-3
            // Step 4
            if (parameters.verbosity >= 1)
              std::cout << "    ----> DONE with the merging. Now running the "
                           "new track fit."
                        << std::endl;
            lar_content::LArPfoHelper::GetSlidingFitTrajectory(
                &caloHitList_v2, vertexVector, slidingFitHalfWindow,
                parameters.pixelPitch, trackStateVector_v2, &indexVector_v2,
                true);
        ...
        lar_content::LArTrackStateVector trackStateVector_out =
            (parameters.voxelizeZ && trackStateSuccess_v2) ? trackStateVector_v2
                                                           : trackStateVector;
        if (parameters.verbosity >= 1)
          std::cout << "    INFO: The track state vector we are using for "
                       "calorimetry analysis has "
                    << trackStateVector_out.size() << " points." << std::endl;

        // Extract the track fit info
        if (!trackStateSuccess ||
            trackStateVector.size() < minTrajectoryPoints) {
          trkStartX.push_back(-9999.);
          trkStartY.push_back(-9999.);
          trkStartZ.push_back(-9999.);
          trkStartDirX.push_back(1.);
          ...
          const lar_content::LArTrackState &trackStateStart =
              (parameters.useVoxelizedStartStop &&
               (trackStateSuccess_v2 &&
                trackStateVector_out.size() >= minTrajectoryPoints))
          ...
          if (trackStateStart.GetPosition().GetX() <
              xBoundaries[0] + parameters.ContainDistX)
            thisTrackContained = false;
          else if (trackStateStart.GetPosition().GetX() >
                   xBoundaries[1] - parameters.ContainDistX)
            thisTrackContained = false;
          else if (trackStateStart.GetPosition().GetY() <
                   yBoundaries[0] + parameters.ContainDistY)
            thisTrackContained = false;
          else if (trackStateStart.GetPosition().GetY() >
                   yBoundaries[1] - parameters.ContainDistY)
            thisTrackContained = false;
          else if (trackStateStart.GetPosition().GetZ() <
                   zBoundaries[0] + parameters.ContainDistZ)
            thisTrackContained = false;
          else if (trackStateStart.GetPosition().GetZ() >
                   zBoundaries[1] - parameters.ContainDistZ)
            thisTrackContained = false;
          else if (trackStateEnd.GetPosition().GetX() <
                   xBoundaries[0] + parameters.ContainDistX)
            thisTrackContained = false;
          else if (trackStateEnd.GetPosition().GetX() >
                   xBoundaries[1] - parameters.ContainDistX)
            thisTrackContained = false;
          else if (trackStateEnd.GetPosition().GetY() <
                   yBoundaries[0] + parameters.ContainDistY)
            thisTrackContained = false;
          else if (trackStateEnd.GetPosition().GetY() >
                   yBoundaries[1] - parameters.ContainDistY)
            thisTrackContained = false;
          else if (trackStateEnd.GetPosition().GetZ() <
                   zBoundaries[0] + parameters.ContainDistZ)
            thisTrackContained = false;
          else if (trackStateEnd.GetPosition().GetZ() >
                   zBoundaries[1] - parameters.ContainDistZ)
          ...
          // -- check start
          if (trackStateStart.GetPosition().GetX() - xBoundaries[0] <
              minDistFromWall)
            minDistFromWall =
                trackStateStart.GetPosition().GetX() - xBoundaries[0];
          if (xBoundaries[1] - trackStateStart.GetPosition().GetX() <
              minDistFromWall)
            minDistFromWall =
                xBoundaries[1] - trackStateStart.GetPosition().GetX();
          if (trackStateStart.GetPosition().GetY() - yBoundaries[0] <
              minDistFromWall)
            minDistFromWall =
                trackStateStart.GetPosition().GetY() - yBoundaries[0];
          if (yBoundaries[1] - trackStateStart.GetPosition().GetY() <
              minDistFromWall)
            minDistFromWall =
                yBoundaries[1] - trackStateStart.GetPosition().GetY();
          if (trackStateStart.GetPosition().GetZ() - zBoundaries[0] <
              minDistFromWall)
            minDistFromWall =
                trackStateStart.GetPosition().GetZ() - zBoundaries[0];
          if (zBoundaries[1] - trackStateStart.GetPosition().GetZ() <
              minDistFromWall)
            minDistFromWall =
                zBoundaries[1] - trackStateStart.GetPosition().GetZ();
          // -- check end
          if (trackStateEnd.GetPosition().GetX() - xBoundaries[0] <
              minDistFromWall)
            minDistFromWall =
                trackStateEnd.GetPosition().GetX() - xBoundaries[0];
          if (xBoundaries[1] - trackStateEnd.GetPosition().GetX() <
              minDistFromWall)
            minDistFromWall =
                xBoundaries[1] - trackStateEnd.GetPosition().GetX();
          if (trackStateEnd.GetPosition().GetY() - yBoundaries[0] <
              minDistFromWall)
            minDistFromWall =
                trackStateEnd.GetPosition().GetY() - yBoundaries[0];
          if (yBoundaries[1] - trackStateEnd.GetPosition().GetY() <
              minDistFromWall)
            minDistFromWall =
                yBoundaries[1] - trackStateEnd.GetPosition().GetY();
          if (trackStateEnd.GetPosition().GetZ() - zBoundaries[0] <
              minDistFromWall)
            minDistFromWall =
                trackStateEnd.GetPosition().GetZ() - zBoundaries[0];
          if (zBoundaries[1] - trackStateEnd.GetPosition().GetZ() <
              minDistFromWall)
            minDistFromWall =
                zBoundaries[1] - trackStateEnd.GetPosition().GetZ();
          trkWallDistance.push_back(minDistFromWall);

          float trklength = 0.;
          // Get the length going point to point
          if (parameters.useVoxelizedStartStop &&
              (trackStateSuccess_v2 &&
               trackStateVector_out.size() >= minTrajectoryPoints)) {
            for (unsigned int idxPt = 0;
                 idxPt < trackStateVector_out.size() - 1; ++idxPt) {
              const lar_content::LArTrackState &trackState =
                  trackStateVector_out.at(idxPt);
              const lar_content::LArTrackState &trackStateNext =
                  trackStateVector_out.at(idxPt + 1);
              trklength +=
                  std::sqrt(trackState.GetPosition().GetDistanceSquared(
                      trackStateNext.GetPosition()));
            }
          } else {
            for (unsigned int idxPt = 0; idxPt < trackStateVector.size() - 1;
                 ++idxPt) {
              const lar_content::LArTrackState &trackState =
                  trackStateVector.at(idxPt);
              const lar_content::LArTrackState &trackStateNext =
                  trackStateVector.at(idxPt + 1);
              trklength +=
                  std::sqrt(trackState.GetPosition().GetDistanceSquared(
                      trackStateNext.GetPosition()));
            }
          ...
          if (KEFromLength_muon > 0.) {
            ...
            trk_pFromLength_muon.push_back(
                std::sqrt((KEFromLength_muon * KEFromLength_muon) +
                          (2 * 105.7 * KEFromLength_muon)) /
                1000.);
          } else {
            trk_KEFromLength_muon.push_back(0.);
            trk_pFromLength_muon.push_back(0.);
          ...
          // Track calorimetry --> very rough first pass basically reimplemented
          // from other test branch: ! Consider the first and last points, but
          // here we only have one side of dx ! Does not do spacecharge,
          // diffusion, etc. corrections at least yet
          float summedTrkE = 0.;
          float summedQinTrk = 0.;

          // If we aren't going to do the track calorimetry, then say the track
          // caloE = 0
          ...
          if (trackStateVector_out.size() >= minTrajectoryPoints) {
            float lengthSoFar = 0.;
            for (unsigned int idxPt = 0; idxPt < trackStateVector_out.size();
                 ++idxPt) {
              const lar_content::LArTrackState &trackState =
                  trackStateVector_out.at(idxPt);
              ...
              // residual range
              if (idxPt > 0) {
                const lar_content::LArTrackState &trackStatePrev =
                    trackStateVector_out.at(idxPt - 1);
                lengthSoFar +=
                    std::sqrt(trackStatePrev.GetPosition().GetDistanceSquared(
                        trackState.GetPosition()));
              }
              ...
              if (idxPt == 0) {
                if (idxPt < trackStateVector_out.size() - 1) {
                  const lar_content::LArTrackState &trackStateNext =
                      trackStateVector_out.at(idxPt + 1);
                  hitdx = std::sqrt(trackState.GetPosition().GetDistanceSquared(
                              trackStateNext.GetPosition())) /
                          2.;
                }
              } else {
              ...
              if (parameters.fShouldCorrectLifetime) {
                hitdQdx *=
                    (1000. *
                     LifetimeCorrectionFactor(
                         posAnodes, trackState.GetPosition().GetX(),
                         parameters.fElectronLifetime,
                         parameters
                             .fElectronDriftSpeed)); // turn ke- to e- and do
                                                     // lifetime correction
              ...
              summedTrkE +=
                  hitdEdx * hitdx; // sum up the energy along the track

            } // loop points
            // And now that we have dE/dx for all points, we can use the sum of
            // that all to get the track calo E
            trackFitTrackCaloE.push_back(summedTrkE);
            // And calculate the total VisE for the track:
            trackFitVisE.push_back(
                eVisWithRecombination(parameters, summedQinTrk));

            // Particle ID here
            if (parameters.fShouldRunPID) {
              if (parameters.fPIDAlgChi2PID) {
                // as in
                // https://github.com/LArSoft/larana/blob/develop/larana/ParticleIdentification/Chi2PIDAlg.cxx#L90
                float chi2pro = 0.;
                float chi2ka = 0.;
                float chi2pi = 0.;
                float chi2mu = 0.;
                int nbins_dedx_range =
                    parameters.templatesdEdxRR.at("proton")->GetNbinsX();
                int npts = 0;
                for (unsigned int idxCaloPt = 0;
                     idxCaloPt < trackVecDEDX.size(); ++idxCaloPt) {
                  ...
                  if (trackVecDEDX[idxCaloPt] < parameters.fChi2RestrictDEDXLo)
                    continue; // also, optionally restrict unexpected low dE/dx.
                              // By default just requires it to be positive.
                  if (parameters.fChi2RestrictDX &&
                      (trackVecDX[idxCaloPt] < parameters.fChi2RestrictDXLo ||
                       (parameters.fChi2RestrictDXHi > 0. &&
                        trackVecDX[idxCaloPt] >
                            parameters.fChi2RestrictDXHi))) {
                    continue; // optionally skip this point if dx too
                              // small/large
                  }
                  int bin = parameters.templatesdEdxRR.at("proton")->FindBin(
                      trackVecRR[idxCaloPt]);
                  if (bin >= 1 && bin <= nbins_dedx_range) {
                    // Content
                    float bincpro =
                        parameters.templatesdEdxRR.at("proton")->GetBinContent(
                            bin);
                    if (bincpro < 1e-6)
                      bincpro = (parameters.templatesdEdxRR.at("proton")
                                     ->GetBinContent(bin - 1) +
                                 parameters.templatesdEdxRR.at("proton")
                                     ->GetBinContent(bin + 1)) /
                                2.;
                    float bincka =
                        parameters.templatesdEdxRR.at("kaon")->GetBinContent(
                            bin);
                    if (bincka < 1e-6)
                      bincka =
                          (parameters.templatesdEdxRR.at("kaon")->GetBinContent(
                          ...
                          2.;
                    float bincpi =
                        parameters.templatesdEdxRR.at("pion")->GetBinContent(
                            bin);
                    if (bincpi < 1e-6)
                      bincpi =
                          (parameters.templatesdEdxRR.at("pion")->GetBinContent(
                          ...
                          2.;
                    float bincmu =
                        parameters.templatesdEdxRR.at("muon")->GetBinContent(
                            bin);
                    if (bincmu < 1e-6)
                      bincmu =
                          (parameters.templatesdEdxRR.at("muon")->GetBinContent(
                    ...
                    // Error
                    float binepro =
                        parameters.templatesdEdxRR.at("proton")->GetBinError(
                            bin);
                    if (binepro < 1e-6)
                      binepro =
                          (parameters.templatesdEdxRR.at("proton")->GetBinError(
                          ...
                          2.;
                    float bineka =
                        parameters.templatesdEdxRR.at("kaon")->GetBinError(bin);
                    if (bineka < 1e-6)
                      bineka =
                          (parameters.templatesdEdxRR.at("kaon")->GetBinError(
                               bin - 1) +
                           parameters.templatesdEdxRR.at("kaon")->GetBinError(
                               bin + 1)) /
                          2.;
                    float binepi =
                        parameters.templatesdEdxRR.at("pion")->GetBinError(bin);
                    if (binepi < 1e-6)
                      binepi =
                          (parameters.templatesdEdxRR.at("pion")->GetBinError(
                               bin - 1) +
                           parameters.templatesdEdxRR.at("pion")->GetBinError(
                               bin + 1)) /
                          2.;
                    float binemu =
                        parameters.templatesdEdxRR.at("muon")->GetBinError(bin);
                    if (binemu < 1e-6)
                      binemu =
                          (parameters.templatesdEdxRR.at("muon")->GetBinError(
                               bin - 1) +
                           parameters.templatesdEdxRR.at("muon")->GetBinError(
                               bin + 1)) /
                          2.;
                    float errdedx = 0.04231 + 0.0001783 *
                                                  trackVecDEDX[idxCaloPt] *
                                                  trackVecDEDX[idxCaloPt];
                    ...
                    if (!parameters.fApplyCalibrationFudgeFactor &&
                        parameters.fApplyCalibrationFudgeFactor_PID)
                      thisPointDEDX *= parameters.fCalibrationFudgeFactor;
                    chi2pro += std::pow(thisPointDEDX - bincpro, 2) /
                               (binepro * binepro + errdedx_square);
                    chi2ka += std::pow(thisPointDEDX - bincka, 2) /
                              (bineka * bineka + errdedx_square);
                    chi2pi += std::pow(thisPointDEDX - bincpi, 2) /
                              (binepi * binepi + errdedx_square);
                    chi2mu += std::pow(thisPointDEDX - bincmu, 2) /
                              (binemu * binemu + errdedx_square);
                    npts += 1;
                  } // within bins
                }   // loop calo points
        ...
        if (!filledPID) {
          // if PID isn't filled then we need to add in the defaults for this
          // track
          pid_pdg.push_back(0);
          pid_ndf.push_back(0);
          pid_muScore.push_back(-5.);
      ...
      if (parameters.runShowerFit && (parameters.trackScoreCut < 0. ||
                                      trackScore < parameters.trackScoreCut)) {
        ...
        lar_content::LArPcaHelper::RunPca(caloHitList, centroid, eigenValues,
                                          eigenVecs);

        // Define directions to be positive
        CartesianVector axisDirection(eigenVecs.at(0).GetZ() > 0.f
                                          ? eigenVecs.at(0)
                                          : eigenVecs.at(0) * -1.f);
        ...
        // Find projections for each hit along the primary axis and save them
        // into a map from least to greatest

        for (const CaloHit *const pCaloHit3D : caloHitList) {

          projection = axisDirection.GetDotProduct(
              pCaloHit3D->GetPositionVector() - centroid);
        ...
        if (fabs(showerStartHitProjectionValue -
        ...
        CartesianVector showerStartPosition =
            centroid + axisDirection * showerStartHitProjectionValue;

        float showerStartLength = parameters.showerStartLength;
        int showerStartWidth = parameters.showerStartWidth;

        // Define shower direction as a vector passing through both the start
        ...
        for (const auto &iMapEntry : projectionMap) {
          CartesianVector start_to_hit_dir =
              (iMapEntry.second - showerStartHitPos);
        ...
        if (avg_low > avg_high) {
          // flip PCA axis
          CartesianVector axisDirectionFlipped(-axisDirection.GetX(),
                                               -axisDirection.GetY(),
                                               -axisDirection.GetZ());

          for (auto iMapEntry = projectionMap.rbegin();
               iMapEntry != projectionMap.rend(); ++iMapEntry) {
            float hit_i_j_dist;
            CartesianVector hit_i_pos(0.f, 0.f, 0.f), hit_j_pos(0.f, 0.f, 0.f);
            proximityHitsCounter = 0;
            hit_i_pos = iMapEntry->second;
            ...
            for (auto jMapEntry = projectionMap.rbegin();
                 jMapEntry != projectionMap.rend(); ++jMapEntry) {
              hit_j_pos = jMapEntry->second;
          ...
          showerStartPosition =
              centroid + axisDirectionFlipped * showerStartHitProjectionValue;
        ...
        for (const CaloHit *const pShowerStartCaloHit3D : caloHitList) {
          showerStartPCAProjection =
              centroid + (showerDirection * showerStartHitProjectionValue);
          showerStartCurrentHit = pShowerStartCaloHit3D->GetPositionVector();
          totalCharge +=
              (pShowerStartCaloHit3D->GetInputEnergy()) *
              LifetimeCorrectionFactor(posAnodes, showerStartCurrentHit.GetX(),
                                       parameters.fElectronLifetime,
                                       parameters.fElectronDriftSpeed);
          if (showerStartPCAProjection == showerStartCurrentHit) {
            continue;
          } else {
            CartesianVector input =
                showerStartCurrentHit - showerStartPCAProjection;
            if (input.GetMagnitude() < 0.001) {
              continue;
            }
            hitPCAOpeningAngle = showerDirection.GetOpeningAngle(
                showerStartCurrentHit - showerStartPCAProjection);
            distanceFromShowerStart =
                std::sqrt(showerStartCurrentHit.GetDistanceSquared(
                    showerStartPCAProjection));
            hitPositionAlongAxis =
                distanceFromShowerStart * (std::cos(hitPCAOpeningAngle));
            hitPositionFromAxis =
                distanceFromShowerStart * std::sin(hitPCAOpeningAngle);
          }

          if (hitPositionAlongAxis > 0 &&
              hitPositionAlongAxis < showerStartLength &&
              hitPositionFromAxis < showerStartWidth) {
            showerStartCaloHitList.push_back(pShowerStartCaloHit3D);
            chargeStartPoints += pShowerStartCaloHit3D->GetInputEnergy() *
                                 LifetimeCorrectionFactor(
                                     posAnodes, showerStartCurrentHit.GetX(),
                                     parameters.fElectronLifetime,
                                     parameters.fElectronDriftSpeed);
        ...
        float energyStartPoints = chargeStartPoints * (1000) * (23.6 / 1e6) *
                                  (parameters.energyRecombinationShower) *
                                  (parameters.correctionFactorShower);
        float energyTotal = totalCharge * (1000) * (23.6 / 1e6) *
                            (parameters.energyRecombinationShower) *
                            (parameters.correctionFactorShower);
    ...
    // Fill track branches: this will fill per particle values with default
    // values if track fit is not run or is skipped
    fOut.FillTrackBranches(trkStartX, trkStartY, trkStartZ, trkStartDirX,
                           trkStartDirY, trkStartDirZ, trkEndX, trkEndY,
                           trkEndZ, trkEndDirX, trkEndDirY, trkEndDirZ, trkLen,
                           trkContained, trkWallDistance, trk_KEFromLength_muon,
                           trk_KEFromLength_proton, trk_pFromLength_muon,
                           trk_pFromLength_proton);
    fOut.FillTrackCaloBranches(parameters, trackFitTrackCaloE, trackFitVisE,
                               trackFitSliceId, trackFitPfoId, trackFitX,
                               trackFitY, trackFitZ, trackFitQ, trackFitRR,
                               trackFitdx, trackFitdQdx, trackFitdEdx);
    fOut.FillTrackPID(pid_pdg, pid_ndf, pid_muScore, pid_piScore, pid_kScore,
                      pid_proScore);

    fOut.FillShowerBranches(
        shwrCentroidX, shwrCentroidY, shwrCentroidZ, shwrStartX, shwrStartY,
        shwrStartZ, shwrDirX, shwrDirY, shwrDirZ, shwrLen, shwrSliceId,
        shwrClusterId, shwrdEdx, shwrEnergy, shwrEndX, shwrEndY, shwrEndZ);
...
bool ReadSettings(ParameterStruct &parameters) {
  ...
  if (!xmlDocument.LoadFile()) {
    std::cout << "XML document (" << parameters.xmlName
              << ") not loaded. Returning." << std::endl;
    return false;
  }

  const TiXmlHandle xmlDocumentHandle(&xmlDocument);
  const TiXmlHandle xmlHandle(
      TiXmlHandle(xmlDocumentHandle.FirstChildElement().Element()));

  try {
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldRunTrackFit",
                             parameters.runTrackFit));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldRunShowerFit",
                             parameters.runShowerFit));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "TrackScoreCut",
                             parameters.trackScoreCut));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldApplyHitThreshold",
                             parameters.applyThreshold));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ChargeThreshold",
                             parameters.thresholdVal));

    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldVoxelizeZ",
                             parameters.voxelizeZ));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "VoxelHalfWidthZ",
                             parameters.voxelZHW));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "UseVoxelizedStartStop",
                             parameters.useVoxelizedStartStop));

    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShowerStartLength",
                             parameters.showerStartLength));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShowerStartWidth",
                             parameters.showerStartWidth));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "SigmaLength", parameters.sigmaLength));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ProximityHitsThreshold",
                             parameters.proximityHitsThreshold));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ProximityHitsRadius",
                             parameters.proximityHitsRadius));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "EnergyRecombinationShower",
                             parameters.energyRecombinationShower));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "CorrectionFactorShower",
                             parameters.correctionFactorShower));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ContainDistX",
                             parameters.ContainDistX));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ContainDistY",
                             parameters.ContainDistY));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ContainDistZ",
                             parameters.ContainDistZ));

    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldSaveCaloPoints",
                             parameters.fShouldSaveCaloPoints))
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldCorrectLifetime",
                             parameters.fShouldCorrectLifetime));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ElectronLifetime",
                             parameters.fElectronLifetime));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ElectronDriftSpeed",
                             parameters.fElectronDriftSpeed));

    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldCorrectRecombination",
                             parameters.fShouldCorrectRecomb));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "FlowStyleRecombination",
                             parameters.fFlowStyleRecombination));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "BoxRecombination",
                             parameters.fBoxRecombination));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "BirksRecombination",
                             parameters.fBirksRecombination));
    // Check if we have set to do BOTH Box & Birks if correcting lifetime, or if
    // we have set to correct lifetime and chosen NEITHER
    if (parameters.fShouldCorrectRecomb) {
      if (parameters.fBoxRecombination && parameters.fBirksRecombination) {
        std::cout << "BOTH Box and Birks are true. Please set one to false."
                  << std::endl;
        return false;
      }
      if (!parameters.fBoxRecombination && !parameters.fBirksRecombination) {
        std::cout << "BOTH Box and Birks are false. Please set one to true."
                  << std::endl;
        return false;
      }
    }
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "EField", parameters.fEField));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "BirksA", parameters.fBirksA));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "BirksK", parameters.fBirksK));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ApplyCalibrationFudgeFactor",
                             parameters.fApplyCalibrationFudgeFactor));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ApplyCalibrationFudgeFactorPID",
                             parameters.fApplyCalibrationFudgeFactor_PID));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "CalibrationFudgeFactor",
                             parameters.fCalibrationFudgeFactor));

    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "ShouldRunPID",
                             parameters.fShouldRunPID));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "PIDAlgChi2PID",
                             parameters.fPIDAlgChi2PID));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "Chi2RestrictDX",
                             parameters.fChi2RestrictDX));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "Chi2RestrictDXLo",
                             parameters.fChi2RestrictDXLo));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "Chi2RestrictDXHi",
                             parameters.fChi2RestrictDXHi));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "Chi2RestrictDEDXLo",
                             parameters.fChi2RestrictDEDXLo));
    PANDORA_RETURN_RESULT_IF_AND_IF(
        pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=,
        XmlHelper::ReadValue(xmlHandle, "dEdxResTempFile",
                             parameters.fdEdxResTempFile));

    if (parameters.fShouldRunPID && parameters.fPIDAlgChi2PID) {
      // Following scheme as in LArSoft
      // larana/ParticleIdentification/Chi2PIDAlg.cxx
      TFile *tempFile = TFile::Open(parameters.fdEdxResTempFile.c_str());
      parameters.templatesdEdxRR["muon"] =
          (TProfile *)tempFile->Get("dedx_range_mu");
      parameters.templatesdEdxRR["pion"] =
          (TProfile *)tempFile->Get("dedx_range_pi");
      parameters.templatesdEdxRR["proton"] =
          (TProfile *)tempFile->Get("dedx_range_pro");
      parameters.templatesdEdxRR["kaon"] =
          (TProfile *)tempFile->Get("dedx_range_ka");
      try {
        if (parameters.templatesdEdxRR.at("muon")->GetNbinsX() < 1 &&
          ...
          std::cout << "Not enough data in dEdx vs RR templates. Returning"
                    << std::endl;
          return false;
        }
      } catch (StatusCodeException &statusCodeException) {
        std::cout << "Issue loading dEdx vs RR templates. Returning"
                  << std::endl;
        return false;
      }
    }
    ...
    std::cout
        << "Failed to initialized parameters in the XML file. Status code "
        << statusCodeException.GetStatusCode() << ". Returning." << std::endl;
    return false;
  }
...
bool PrintOptions() {
  std::cout << std::endl
            << "./bin/PandoraOuterface -f [file name] -o [out name] -p [pixel "
               "pitch] -c [track score cut] -t -s"
            << std::endl
